...
- [ ] 매칭개수, 상금, 보너스 필요 여부를 가진 열거형 클래스 생성
- [ ] 각 등수에 해당하는 상금을 가진다.
- [ ] 매칭개수에 따른 랭킹을 반환하는 메소드를 가진다.

#### 순위 테스트

- [ ] 매칭 개수와 보너스 번호 일치 여부에 따른 등수 확인 테스트

### 로또 결과

#### 구현

- [ ] 로또에서 당첨 숫자가 포함되어있는지 확인한다.
- [ ] 로또에서 보너스 숫자가 포함되는지 확인한다.
- [ ] 총 상금을 구하는 메소드를 가진다.

#### 로또 결과 테스트

- [ ] 올바른 당첨 통계를 반환하는지 확인한다.
- [ ] 올바른 당첨금액을 반환하는지 확인한다.
- [ ] 닥첨 번호가 없는경우 낙첨으로 처리하는지 확인한다.
- [ ] 보너스 여부에 따라 2,3등을 구분하는지 확인한다.

### 로또 통계

#### 구현

- [ ] 랭킹과 당첨횟수를 위한 카운트를 가진 map을 가진다.
- [ ] 당첨 카운트를 증가시키는 메소드를 가진다.
- [ ] 당첨 카운트를 반환하는 메소드를 가진다.
- [ ] 총 상금금액을 계산 하여 반환하는 메소드를 가진다.

#### 로또 통계 테스트

- [ ] 랭킹 카운트를 증가 여부 확인 테스트
- [ ] 올바른 총 상금 계산 확인 테스트
...
- [ ] 당첨금액을 검증한다.
    - [ ] 음수인지
- [ ] 구매금액, 당첨금액을 통해 수익률을 구한다.

#### 수익률 테스트

- [ ] 구매금액이 양수가 아닌경우 예외 발생 테스트
- [ ] 구매금액이 1000원 단위가 아닌 경우 예외 발생 테스트
- [ ] 당첨금액이 음수인 경우 예외 발생 테스트
- [ ] 올바른 구매금액, 당첨금액이 입력된 경우 올바른 수익률 반환 테스트
