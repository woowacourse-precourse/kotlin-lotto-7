...
## 🧷 추가로 정의한 사항
- 구입금액은 Long 타입으로 저장 가능한만큼 입력이 가능하다.
- 모든 입력 상황에서 숫자와 콤마 이외의 문자나 공백이 들어갈 경우 예외 처리된다.
- 발행한 로또의 숫자는 오름차순으로 출력하지만 당첨번호의 입력은 순서에 상관없다.
...
9. 잘못된 입력에 대한 예외 처리
    ...
    - 보너스 번호 입력 검사목록
        - 보너스 번호는 1~45 이내의 정수이며 당첨번호와 중복되지 않는가?

## 📝 테스트
- unitTest 내의 GameServiceTest와 InputValidatorTest에서 단위 테스트를 수행한다
- LottoGameWholeTest에서 프로그램 전체 입력과 오류 테스트를 진행한다.
