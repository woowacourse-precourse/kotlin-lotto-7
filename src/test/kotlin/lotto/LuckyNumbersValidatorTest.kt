...
class LuckyNumbersValidatorTest {
    @ParameterizedTest
    @ValueSource(strings = [""])
    @DisplayName("당첨 번호 - 빈 문자")
    fun `보너스 번호에 빈 문자열이 들어오면 예외가 발생한다`(input: String) {
        val exception = assertThrows<IllegalArgumentException> {
            LuckyNumbersValidator(input).validateLuckyNumbers()
        }
        assert(exception.message == LuckyNumbersErrorType.EMPTY_INPUT.errorMessage)
    }
    ...
    fun `당첨 번호에 소수가 들어오면 예외가 발생한다`(input: String) {
        ...
        assert(exception.message == LuckyNumbersErrorType.NOT_DECIMAL.errorMessage)
    }
    ...
    fun `당첨 번호에 구분자(,)가 안 들어오면 예외가 발생한다`(input: String) {
        ...
        assert(exception.message == LuckyNumbersErrorType.N0_COMMA.errorMessage)
    }
    ...
    fun `당첨 번호에 정수가 아닌 것이 들어오면 예외가 발생한다`(input: String) {
        ...
        assert(exception.message == LuckyNumbersErrorType.NOT_INTEGER.errorMessage)
    }
    ...
    fun `당첨 번호에 1 ~ 45사이가 아닌 것이 들어오면 예외가 발생한다`(input: String) {
        ...
        assert(exception.message == LuckyNumbersErrorType.LUCKY_NUMBERS_RANGE.errorMessage)
    }
    ...
    fun `당첨 번호에 중복된 번호가 들어오면 예외가 발생한다`(input: String) {
        ...
        assert(exception.message == LuckyNumbersErrorType.NO_DUPLICATE_LUCKY_NUMBERS.errorMessage)  }
    ...
    fun `당첨 번호에 번호 6개가 안 들어오면 예외가 발생한다`(input: String) {
        ...
        assert(exception.message == LuckyNumbersErrorType.LUCKY_NUMBERS_6.errorMessage)  }
}