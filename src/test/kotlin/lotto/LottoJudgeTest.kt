...
class LottoJudgeTest {
    ...
    fun `로또 당첨 번호는 6개 입력되어야 한다`() {
        ...
        assertThat(exceptionForNumbersCountIsSeven.message).contains(INPUT_WINNER_NUMBER_COUNT_ERROR.getMessage())

        val exceptionForNumbersCountIsFive = `로또 당첨 번호가 5개인 경우 예외가 발생해야 한다`()
        assertThat(exceptionForNumbersCountIsFive.message).contains(INPUT_WINNER_NUMBER_COUNT_ERROR.getMessage())
    }
    ...
    fun `로또 당첨 번호는 1 이상, 45 이하로 입력되어야 한다`() {
        ...
        assertThat(exceptionForNumbersMinus.message).contains(INPUT_WINNER_NUMBER_OUT_OF_RANGE_ERROR.getMessage())

        val exceptionForNumbersAboveFortyFive = `로또 당첨 번호가 45초과인 경우 예외가 발생해야 한다`()
        assertThat(exceptionForNumbersAboveFortyFive.message).contains(INPUT_WINNER_NUMBER_OUT_OF_RANGE_ERROR.getMessage())
    }
    ...
    fun `로또 당첨 번호에 중복된 숫자가 있는 경우 예외가 발생해야 한다`() {
        ...
        assertThat(exception.message).contains(INPUT_WINNER_NUMBER_NO_DUPLICATE.getMessage())
    }
    ...
    fun `보너스 번호는 1 이상, 45 이하로 입력되어야 한다`() {
        ...
        assertThat(exceptionForBonusNumberLessThanOne.message).contains(INPUT_WINNER_NUMBER_OUT_OF_RANGE_ERROR.getMessage())

        val exceptionForBonusNumberGreaterThanFortyFive = `보너스 번호가 45초과인 경우 예외가 발생해야 한다`()
        assertThat(exceptionForBonusNumberGreaterThanFortyFive.message).contains(INPUT_WINNER_NUMBER_OUT_OF_RANGE_ERROR.getMessage())
    }
