...
class LottoGame {
    fun start() {
        ...
        val lottos = generateLottos(purchaseAmount / LottoConfig.LOTTO_PRICE)
        printPurchaseResult(lottos)

        val winningNumbers = getWinningNumbers()
        val bonusNumber = getBonusNumber(winningNumbers)
        val result = calculateResults(lottos, winningNumbers, bonusNumber)
        printResults(result)
    }
    ...
    private fun generateLottos(count: Int): List<Lotto> {
        val lottos = ArrayList<Lotto>(count)
        repeat(count) {
